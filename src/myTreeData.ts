...
export class MyTreeData implements TreeDataProvider<MyTreeItem>{
    ...
    getChildren(element?: MyTreeItem | undefined): ProviderResult<MyTreeItem[]>{
        ...
        // }
        if(element === undefined){
            return Promise.resolve(this.searchFiles(this.rootPath,null, 1));
        }
        else{
            return Promise.resolve(this.searchFiles(element.parentPath, element.label,element.level));
        }
    }
    
    //查找文件，文件夹
    private searchFiles(parentPath: string, keyword:string|null ,level: number): MyTreeItem[] {
